...
def ResNet50_reference(X, classes=10):
    ...
    # stage 1
    x = tf.layers.conv2d(X, filters=64, kernel_size=(3, 3), strides=(1, 1), name='conv1')
    ...
    # stage 2
    x = convolutional_block(x, kernel_size=3, filters=[64, 64, 128], stage=2, block='a', stride=1)
    x = identity_block(x, 3, [64, 64, 128], stage=2, block='b')
    x = identity_block(x, 3, [64, 64, 128], stage=2, block='c')

    # stage 3
    x = convolutional_block(x, kernel_size=3, filters=[128,128,256],
                                            stage=3, block='a', stride=1)
    ...
    # stage 4
    x = convolutional_block(x, kernel_size=3, filters=[256, 256, 512], stage=4, block='a', stride=1)
    ...
    # stage 5
    x = convolutional_block(x,kernel_size=3,filters=[512, 512, 1024], stage=5, block='a', stride=1)
...
def main():
    ...
    with tf.Session() as sess:
        ...
        mini_batches = random_mini_batches(X_train, Y_train, mini_batch_size=500)
